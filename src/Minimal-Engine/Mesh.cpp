...
Mesh::Mesh (std::vector<Vertex> vertices, std::vector<GLuint> indices) noexcept
{
  ...
  glBufferData (GL_ARRAY_BUFFER, numVertices * sizeof (Mesh::Vertex),
                std::move (vertices).data (), GL_STATIC_DRAW);

  glBindBuffer (GL_ELEMENT_ARRAY_BUFFER, m_ebo);
  glBufferData (GL_ELEMENT_ARRAY_BUFFER, m_numVertex * sizeof (GLuint),
                std::move (indices).data (), GL_STATIC_DRAW);
...
void
Mesh::render (GLuint type) const noexcept
{
  glDrawElements (type, m_numVertex, GL_UNSIGNED_INT, nullptr);
}

void
Mesh::unbind () const noexcept
{
