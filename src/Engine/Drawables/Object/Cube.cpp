...
namespace daft::engine {
    ...
    Cube::Cube(float radius, int resolution, daft::engine::Composite *parent, std::string name) : Object(parent,
                                                                                                         std::move(
                                                                                                                 name)),
                                                                                                  m_radius{radius},
                                                                                                  m_resolution{
            ...
            for (int y = 0; y < m_resolution; ++y) {
                for (int x = 0; x < m_resolution; ++x) {
                    ...
                    glm::vec3 pointOnUnitCube =
                            dir + (percent.x - 0.5f) * 2.f * axisA + (percent.y - 0.5f) * 2.f * axisB;
