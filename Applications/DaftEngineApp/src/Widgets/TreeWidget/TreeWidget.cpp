...
void TreeWidget::addNode(QStandardItem *parent, engine::Composite *node) {
    for (auto &drawable : node->drawables()) {
        ...
        else if (drawable->getType() == engine::Drawable::Type::Metaballs)
            addNode(item, (engine::Metaballs *)drawable.get());
    }
}
...
void TreeWidget::addNode(QStandardItem *parent, engine::Metaballs *metaballs) {
    for (size_t i = 0; i < metaballs->balls().size(); ++i) {
        auto *item = new QStandardItem(("Ball" + std::to_string(i)).c_str());
        item->setData(QColor(0xf2f2f2), Qt::TextColorRole);
        item->setEditable(false);
        parent->appendRow(item);
    }
